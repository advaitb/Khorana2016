...
plt.title( "FOXA1 regression vs NEBL" )

y = np.array(avgs["NEBL"])
x = np.array(avgs["FOXA1"])
y1 = np.array(avgs1["NEBL"])
x1 = np.array(avgs1["FOXA1"])
y2 = np.array(avgs2["NEBL"])
x2 = np.array(avgs2["FOXA1"])
...
plt.legend([s1,s2,s3] , ['ER', 'NEBL','IGF1R','NEBL', 'RAR', 'NEBL'], loc='lower right')
plt.ylim((0,1))
plt.xlim((0,1))
plt.xlabel('Percent Probability of FOXA1')
